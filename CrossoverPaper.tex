...
%The linewidth is reduced from the Schawlow Townes linewidth as common
%phase information starts to become stored in the atoms, but that the
%power is also reduced.  We find that a system is more sensitive to
%atomic dephasing and spontaneous emission in the superradiant region,
  %and less sensitive in the lasing region.
...
%We find that our c-number Langevin simulations and analytic Langevin
%theory agree well, that all three theories agree qualitatively, but
%that there is a consistent $25\%$  difference in the linewidth between
%the analytic Langevin and cumulant theories. This difference can be
%seen explicitly by comparing equations (\ref{LWHaken}) (\ref{LWSimple})
%in the limit $\gamma=\frac{1}{T_2}=0$. The only difference is that
%Eq.~(\ref{LWHaken}) has the numerical pre-factor $ \frac{C+1}{2(C-1)}$,
%while Eq.~(\ref{LWSimple}) has the numerical prefactor $
%\frac{C^2+1}{C(C-1)}$.  For $C>1$, this difference in pre-factor is
%never more than a $33\%$. Therefore, the qualitative behavior of the
%cumulant theory can be relied on.
...
%Therefore the $25\%$ disagreement between the Langevin and cumulant
%theories for large N is not surprising.
...
%We find that the linewidth is reduced from the Schawlow Townes
%linewidth as the crossover is traversed from the lasing into the
%intermediate and superradiance regimes. We also find that the output
%power grows only gradually as the crossover is traversed from
%superradiance to lasing.
