...
quantum phase diffusion model, and a mean-field
...
and correlations play an important role in the the noise properties of the system
(e.g.\ the linewidth of the generated light), but can be modelled semi-classically.  Comparison with the
...
In this section, we present results throughout the crossover from lasing to superradiance for the field intensity and linewidth, and for the atomic inversion and correlations. We begin by comparing different solution
...
semiclassical method established we then apply it to experimentally relevant systems with large atom number.
...
mean field Langevin method, the phase diffusion method, the {\it c}-number Langevin method, and exact SU(4) Monte-Carlo simulations
...
reference~\cite{PhysRevA.87.062101}, here we extend the SU(4) method
to allow us to simulate systems in which there are large numbers of
photons as well as large numbers of atoms. We do this by unraveling
the quantum master equation into quantum trajectories---a standard
method in quantum optics. The unusual feature here is that the
unraveling is performed in the Louville space rather than in Hilbert
space, because it is in Louville space that the SU(4) method operates.
